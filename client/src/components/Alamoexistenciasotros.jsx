...
class Alamoexistenciasotros extends Component {
    ...
    componentDidMount(){
        ...
        //Traemos existencias
        fetch('http://localhost:3001/api/leerotrosalamogeneral', requestOptions)
    ...
    onChangeFiltroKeyWord = (e) => {
        ...
        fetch('http://localhost:3001/api/leerotrosalamogeneral', requestOptions)
          ...
          fetch('http://localhost:3001/api/leerexistenciasotrosfiltro/' + e.target.value.toUpperCase(), requestOptions)
              .then(response => response.json())
              .then(data => {
                if(typeof data.err !== 'undefined' && data.err.message.length > 0){
                    localStorage.clear();
