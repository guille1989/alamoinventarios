...
class Alamoexistenciasotros extends Component {
    constructor(props) {
        super(props);
        this.state={
            dataExistencias: [],
    ...
    componentDidMount(){
        ...
        fetch('http://www.alamoinventario.com:80/api/leerotrosalamo', requestOptions)
            .then(response => response.json())
            .then(data => {            
                if(typeof data.err !== 'undefined' && data.err.message.length > 0){
                localStorage.clear();
        ...
        if(this.state.responsableRecepcionExistencia === '' || this.state.tapa === '' || this.state.tapa_cantidad === '' || this.state.tapa_lote === '' || this.state.tapa_fecha_recepcion === '' || this.state.tapa_costo === ''){
          ...
          const requestOptions ={
            ...
            body: JSON.stringify({
                  ...
                  otros: this.state.tapa, 
                  //ProveedorInsumo: this.state.tipoExistenciaProveedor, 
                  otros_costo: this.state.tapa_costo,
                  otros_cantidad: this.state.tapa_cantidad, 
                  otros_lote: this.state.tapa_lote, 
    ...
    onChangeFiltroKeyWord = (e) => {
        ...
        fetch('http://www.alamoinventario.com:80/api/leerotrosalamogeneral', requestOptions)
          ...
          fetch('http://www.alamoinventario.com:80/api/leerexistenciasotrosfiltro/' + e.target.value.toUpperCase(), requestOptions)
              .then(response => response.json())
              .then(data => {
                if(typeof data.err !== 'undefined' && data.err.message.length > 0){
                    localStorage.clear();
    ...
    render() {
        return (
            <div>
                <h3>INVENTARIO PLASTICOS Y CARTONES</h3>
                ...
                <br></br> 
                    ...
                    <Table    
                        ...
                        return(
                            ...
                            <td><strong style={{color: 'green'}}>{Number(item.otros ).toLocaleString('us')}</strong></td>
                            </tr>
                            </>
...
export default Alamoexistenciasotros;