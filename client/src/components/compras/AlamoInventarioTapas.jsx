...
class AlamoInventarioTapas extends Component {
    constructor(props) {
        super(props);
        this.state={
            dataExistencias: [],
    ...
    onChangeFiltroKeyWord = (e) => {
        ...
        fetch('http://www.alamoinventario.com:80/api/leerexistenciastapasalamo', requestOptions)
          ...
          fetch('http://www.alamoinventario.com:80/api/leerexistenciastapasfiltro/' + e.target.value.toUpperCase(), requestOptions)
              .then(response => response.json())
              .then(data => {
                if(typeof data.err !== 'undefined' && data.err.message.length > 0){
                    localStorage.clear();
...
export default AlamoInventarioTapas;